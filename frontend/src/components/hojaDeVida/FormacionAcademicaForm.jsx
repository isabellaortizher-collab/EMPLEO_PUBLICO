...
export default function FormacionAcademica({ onGuardado }) {
    ...
    const cargar = async () => {
        try {
            ...
            setMensaje({tipo: "error", texto: "Error al cargar formacion academica"});
        } finally{
            setCargando(false);
        }
    };
    ...
    const handleArchivo = (e) =>{
        ...
        if (f.size > 2 * 1024 * 1024){
            setMensaje({tipo: "error", texto: "El archivo excede el limite de 2 MB."})
            e.target.value = "";
            return;
        }
    ...
    return (
        <div className="hv-form-card">
                    ...
                    {lista.map((item) => (
                        <div key={item._id} className={`hv-lista-item ${item.validado ? "validado" : ""}`}>
                            ...
                            <div className="hv-lista-acciones">
                                {item.soporteUrl && (
                                    <button type="button" className="hv-btn-soporte" onClick={()=> verDocumento(item.soporteUrl)} title="Mostrar Documento">Ver soporte</button>
                                )}
                                {!item.valido && (
                                    <button type="button" className="hv-btn-eliminar" onClick={()=> handleEliminar(item._id)}>Eliminar</button>
                                )}
                                {item.validado && <span className="hv-badge-valido-sm "> Validado </span>}
                            </div>
                        </div>
                    ))}
                </div>
            ...
            {!mostrarForm && (
                <button type="button" className="hv-btn-agregar" onClick={()=> { setMostrarForm(true); setMensaje(null); }}>Agregar formacion</button>
            )}
        ...
        <form className="hv-form hv-form-nuevo" onSubmit={handleSubmit}>
          ...
          <div className="hv-form-row">
            ...
            <div className="hv-field">
              ...
              <input name="tituloObtenido" value={form.tituloObtenido} onChange={handleChange} required />
            </div>
          </div>
          ...
          <div className="hv-form-row">
            ...
            <div className="hv-field">
              ...
              <input name="paisInstitucion" value={form.paisInstitucion} onChange={handleChange} required />
            </div>
          </div>
          ...
          <div className="hv-form-row">
            <div className="hv-field">
              ...
              <input type="date" name="fechaGrado" value={form.fechaGrado} onChange={handleChange} />
            </div>
            {(form.nivel === "PREGRADO") && (
              <div className="hv-field">
                <label>Número de tarjeta profesional (si aplica)</label>
                <input name="tarjetaProfesional" value={form.tarjetaProfesional} onChange={handleChange} placeholder="Ej: 123456-PRO" />
              </div>
            )}
          </div>
 
          {/* HU-008 / HU-013 — Soporte PDF/JPG máx 2MB */}
          <div className="hv-field">
            ...
            />
            <p className="hv-field-hint">📎 Adjunta el diploma, acta de grado o certificado de la institución.</p>
          </div>

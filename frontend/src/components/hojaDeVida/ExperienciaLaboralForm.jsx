...
export default function ExperienciaLaboralForm({ onGuardado }) {
  ...
  const cargar = async () => {
    try {
      ...
      if (data.ok) setLista(data.experiencia || []);
    } catch {
      setMensaje({ tipo: "error", texto: "Error al cargar experiencia laboral." });
    } finally {
      setCargando(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fd.append("fechaRetiro", form.trabajoActual ? "" : form.fechaRetiro);
      fd.append("trabajoActual", form.trabajoActual);
      fd.append("descripcionFunciones", form.descripcionFunciones);
  ...
  const formatFecha = (fecha) =>
    fecha ? new Date(fecha).toLocaleDateString("es-CO") : "Actualmente";
    ...
    <div className="hv-form-card">
      <div className="hv-form-header">
        ...
        <p className="hv-form-sub">
          Incluye experiencia pública, privada y docente con sus certificaciones.
          ...
          {lista.map((item) => (
            <div key={item._id} className={`hv-lista-item ${item.validado ? "validado" : ""}`}>
              <div className="hv-lista-info">
                <span className={`hv-lista-badge tipo-${item.tipo.toLowerCase()}`}>{item.tipo}</span>
                <strong>{item.cargo}</strong>
                <span>{item.entidadEmpresa}</span>
                <span className="hv-lista-fecha">
                  {formatFecha(item.fechaIngreso)} → {item.trabajoActual ? "Actualmente" : formatFecha(item.fechaRetiro)}
                </span>
                {item.descripcionFunciones && (
                  <p className="hv-lista-desc">{item.descripcionFunciones}</p>
                )}
              </div>
              <div className="hv-lista-acciones">
                ...
                {!item.validado && (
                  <button type="button" className="hv-btn-eliminar" onClick={() => handleEliminar(item._id)}>
                    Eliminar
                  </button>
                )}
      ...
      {lista.length === 0 && !mostrarForm && (
        <p className="hv-lista-vacia">Aún no has registrado experiencia laboral.</p>
      )}
      ...
      {mostrarForm && (
        <form className="hv-form hv-form-nuevo" onSubmit={handleSubmit}>
          <div className="hv-form-nuevo-titulo">Nueva experiencia laboral</div>

          <div className="hv-form-row">
            <div className="hv-field">
              <label>Tipo de experiencia <span className="obligatorio">*</span></label>
            ...
            <div className="hv-field">
              <label>Entidad / Empresa <span className="obligatorio">*</span></label>
              <input name="entidadEmpresa" value={form.entidadEmpresa} onChange={handleChange} required />
            </div>
          </div>

          <div className="hv-field">
            <label>Cargo desempeñado <span className="obligatorio">*</span></label>
            <input name="cargo" value={form.cargo} onChange={handleChange} required />
          </div>

          <div className="hv-form-row">
            <div className="hv-field">
              <label>Fecha de ingreso <span className="obligatorio">*</span></label>
          ...
          <div className="hv-field hv-field-check">
            <label>
              <input type="checkbox" name="trabajoActual" checked={form.trabajoActual} onChange={handleChange} />
          ...
          <div className="hv-field">
            <label>Certificación laboral (PDF o JPG, máx. 2 MB)</label>
            <input type="file" accept=".pdf,.jpg,.jpeg" onChange={handleArchivo} ref={fileRef} className="hv-input-file" />
            <p className="hv-field-hint"> Adjunta la certificación laboral o contrato.</p>
          </div>
